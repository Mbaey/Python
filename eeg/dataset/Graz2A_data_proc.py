...
def gdf_2_mat():
	for j in range(1,10):
		...
		test_subject = [j]
		train_dataset = data_gen(test_subject, high_cut_hz=125, low_cut_hz=0)
		...
		train_X = train_X[idx]
		train_y = train_y[idx]
		sio.savemat(r'D:\li\=.=\eeg\hw\nn-STFT\dataset\BCICIV_2a_mat\A0'+str(j)+'.mat', {"train_x": train_X, "train_y": train_y})
...
if __name__ == '__main__':
	for j in range(1,10):
		...
		train_X = train_X[idx]
		train_y = train_y[idx]
		sio.savemat(r'D:\li\=.=\eeg\hw\nn-STFT\dataset\BCICIV_2a_mat\cross_subject_data_'+str(j)+'.mat', {"train_x": train_X, "train_y": train_y, "test_x": test_X, "test_y": test_y})
